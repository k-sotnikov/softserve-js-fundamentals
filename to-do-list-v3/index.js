...
function delItem(event) {
    let itemToDel = event.target.parentElement.previousElementSibling.children[0].innerHTML;
...
function showItems() {
    ...
    for (const i of toDoItems) {
        if (categoryDropdownList.value === i.itemCategory) {
            ...
            // trNew.innerHTML = `
            //     <td>
            //         <input type="checkbox" class="itemCheckbox" ${isChecked}>
            //     </td>
            //     <td>
            ...
            //         <button class="delItemButton">-</button>
            //     </td>
...
function addEventListenerToCheckBoxes() {
    ...
    for (let i = 0; i < checkboxes.length; i++) {
        ...
        checkbox.addEventListener("click", (event) => { 
            let itemNameInHTML = event.target.parentElement.nextElementSibling.children[0].innerHTML;
            let p = event.target.parentElement.nextElementSibling.children[0]
            let itemInArray = toDoItems.find(item => item.itemName === itemNameInHTML);
            ...
            if (event.target.checked) { //згідно статусу робимо item закресленим чи ні
